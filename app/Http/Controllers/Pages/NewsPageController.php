...
class NewsPageController extends Controller
{
    ...
    {
        $post_list = Post::where('type', '2')->orderBy('created_at', 'desc')->paginate(25);
        $posts_arch = Post::where('type', '1')->orderBy('viewed','desc')->take(3)->get();

        $thumb = UploadImage::load('post', '250');

        return view('layouts.common.news')->with(['post_list'=>$post_list, 'thumb'=>$thumb, 'posts_arch'=>$posts_arch]);
    }
    ...
    public function news($id)
    {
        ...
        return view('layouts.common.newspage._news')->with(['post'=>$post, 'thumb'=>$thumb, 'image_user'=>$image_user, 'posts_arch'=>$posts, 'com'=>$com]);
    }
    ...
    public function tags($name)
    {
        ...
        $post_list = $tag->posts()->orderBy('created_at', 'desc')->paginate(25);
        $posts_arch = Post::where('type', '1')->orderBy('viewed','desc')->take(3)->get();

        $thumb = UploadImage::load('post', '250');

        return view('layouts.common.newspage.tags')->with(['tag'=>$tag, 'post_list'=>$post_list, 'thumb'=>$thumb, 'posts_arch'=>$posts_arch]);
    }
